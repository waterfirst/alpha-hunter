...
**가격 모멘텀 필터 (반드시 적용, 2026-05-14 추가):**
- 52주 고점 대비 -30% 이상 하락 종목은 자동 제외 (떨어지는 칼 방지)
- 6개월 수익률 마이너스 종목은 자동 제외 (하락 추세 배제)
- 3개월 수익률이 동종업종 하위 25%면 제외 (상대 모멘텀)
- 예외: 명확한 턴어라운드 카탈리스트(CEO 교체, 구조조정 완료, 규제 해제)가 웹검색으로 확인된 경우에만 예외 허용하되, 보고서에 "⚠️ 역발상 투자 — 모멘텀 필터 예외 적용" 경고를 명시
...
**사후 검증 (2026-05-14 추가):**
- 추천 종목은 portfolio.json에 매수가·목표가·손절가 기록
